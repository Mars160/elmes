...
class ConfigInitializer:
    ...
    def count_tasks_per_scenario(self, task_prompts: Dict[str, Any]) -> Dict[str, int]:
        ...
        for scenario_name, tasks in task_prompts.items():
            if isinstance(tasks, dict):
                # 统计以"task"开头的键的数量
                task_count = len(
                    [key for key in tasks.keys() if key.startswith("task")]
                )
    ...
    def generate_scenarios_config(
        self, scenarios: Dict[str, Any], task_counts: Dict[str, int]
    ) -> Dict[str, Any]:
        """生成scenarios配置"""
        scenarios_config = {}
        scenario_id = 1
    ...
    def print_summary(self, scenarios_config: Dict[str, Any]):
        ...
        for scenario_name, config in scenarios_config.items():
            print(
                f"S{config['id']} - {config['name']} ({scenario_name}): {config['tasks']} tasks"
            )
