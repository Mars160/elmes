...
class Executor(ABC):
    ...
    def _get_default_filename(self, task_id: Any, scenario: str = None) -> str:
        ...
        # 生成差异码（时分秒+毫秒的后3位）
        diff_code = datetime.now().strftime("%H%M%S") + str(int(time.time() * 1000) % 1000).zfill(3)
        
        # 获取场景名称
        scenario_name = scenario or getattr(self.prompt_generator, 'scenario_name', 'unknown')
        
        # 从config.yaml获取场景ID
        scenario_id = self._get_scenario_id(scenario_name)
        
        # 获取模型名称并处理特殊字符
        model_name = getattr(self.backend, "model_name", "unknown").replace("/", "-").replace(":", "-")
