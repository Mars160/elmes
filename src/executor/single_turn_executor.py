...
class SingleTurnExecutor(Executor):
    ...
    def process_response(self, 
        ...
        # 构建简化的结果，只保留指定字段
        result = {
            "scenario": self.prompt_generator.scenario_name if hasattr(self.prompt_generator, 'scenario_name') else "unknown",
    ...
    def save_result(self, 
        ...
        if not filename:
            ...
            filename = self._get_default_filename(result.get("task_id", "unknown"), scenario)
